...
const { supabase } = require("../lib/supabaseAuth")
...
// Helper: find user by email using listUsers + filter
...
class AuthService {
  static async login(email, password, token, provider) {
    ...
    if (provider && token) {
      // Social login (getUser by access token)
      const { data, error } = await supabase.auth.getUser(token)
      if (error || !data.user) {
        throw new Error(
          "Social login token invalid or user not found: " +
            (error ? error.message : "")
        )
      }
      authResponse = {
        data: { user: data.user, session: { access_token: token } },
      }
      ...
      throw new Error(
        "Invalid login credentials or missing social token/provider."
      )
    }
  ...
  static async signupCandidate({
    ...
    try {
      // Find user by email, fallback to create
      let actualUser = await getUserByEmail(supabase, email)
      if (!actualUser) {
        ...
        if (createUserError || !createUserData?.user) {
          throw new Error("Could not create candidate user.")
        }
        actualUser = createUserData.user
      }

      // Confirm email if needed
      if (!actualUser.email_confirmed_at) {
      ...
      try {
        ...
        if (dbError.code === "23505") {
          throw new Error(
            "A candidate profile with this email or user ID already exists."
          )
        }
        console.error("Error inserting into candidate_profiles table:", dbError)
        throw new Error(
          "Failed to create candidate profile due to database error."
        )
      }
    } catch (updateUserError) {
      console.error(
        "Exception during candidate email confirmation:",
  ...
  static async signupRecruiter({
    ...
    let userRecord
    try {
      userRecord = await getUserByEmail(supabase, email)
      if (!userRecord) {
        const { data: createUserData, error: createUserError } =
          await supabase.auth.admin.createUser({
            email,
        ...
        if (createUserError || !createUserData?.user) {
          throw new Error("Could not create user for recruiter signup.")
        }
        userRecord = createUserData.user
      }
    } catch (userError) {
      console.error("User creation/fetch error:", userError)
      throw new Error("Failed to create or fetch user.")
    }

    // 3. Confirm email if not already
    ...
    try {
      ...
      throw new Error(
        "Failed to create recruiter profile due to database error."
      )
    }
  }
  ...
  static async getUserRoleAndCompanyId(userId) {
    try {
      const recruiterQuery = await query(
        "SELECT role, company_id FROM recruiters WHERE user_id = $1",
        [userId]
      )
      if (recruiterQuery.rows.length > 0) {
        return {
          role: recruiterQuery.rows[0].role,
          company_id: recruiterQuery.rows[0].company_id,
        }
      }

      const candidateQuery = await query(
        "SELECT user_id FROM candidate_profiles WHERE user_id = $1",
        [userId]
      )
...
//       const { data: userByEmailData, error: userByEmailError } =
//         await supabaseAdminClient.users.getUserByEmail(email)
//       if (userByEmailError || !userByEmailData.user) {
//         console.error(
//           "Error fetching user by email for confirmation:",
...
//       const { data: updateData, error: updateError } =
//         await supabaseAdminClient.users.updateUserById(
//           actualUserId, // Use the actual UUID from getUserByEmail
//           { email_confirm: true } // This marks the email as confirmed
//         )
//       if (updateError) {
//         console.error(
//           "Error confirming candidate email via backend service role:",
//           updateError.message
//         )
//         throw new Error("Failed to confirm candidate email after signup.")
//       }
//       console.log(
//         `Candidate user ${actualUserId} email confirmed by backend service role.`
//       )
...
//         await supabaseAdminClient.getUserByEmail(email)
//       if (getUserError) throw getUserError

//       if (getUserData?.user) {
//         userRecord = getUserData.user
//       } else {
//         // 2. Create user
//         const { data: createUserData, error: createUserError } =
//           await supabaseAdminClient.createUser({
//             email,
//             password,
//             email_confirm: true,
//           })
...
//       try {
//         const { data: updateUserData, error: updateUserError } =
//           await supabaseAdminClient.updateUserById(userRecord.id, {
//             email_confirm: true,
//           })
//         if (updateUserError) {
//           throw new Error("Failed to confirm recruiter email after signup.")
